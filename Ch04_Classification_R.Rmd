...
Your code:
```{r,echo=TRUE}
Myfit <- glm(SURVIVED_bin ~ AGE_TRUNK + FEMALE + ASTHMA + DIABETES + RENAL_CHRONIC + PATIENT_TYPE + OBESITY, family=binomial(link = "logit"), data = Train_COVID)
summary(Myfit)
#
#
```
...
Your code:
```{r,echo=TRUE}
logit_P = predict(Myfit,Test_COVID,type="response")
logit_Pred <- ifelse(logit_P > 0.5,1,0)
str(Test_COVID$SURVIVED_bin)
head(logit_P)
roc_score <- roc(response = Test_COVID$SURVIVED_bin, predictor = logit_P)
plot(roc_score, main = "ROC curve -- Logistic Regression")
```
...
~~~
Please write your answer in full sentences.
I put some common decies into the function.
...
ggplot(Auto, aes(x=acceleration, y=mpg01)) + geom_point()
```
...
```{r,echo=TRUE}
set.seed(123) 
trainIndex <- createDataPartition(Auto$mpg01, p = 0.8, list = FALSE, times = 1)

trainSet <- Auto[trainIndex, ]

testSet <- Auto[-trainIndex, ]
#
```
...
Your code:
```{r,echo=TRUE}
lda_fit <- lda(mpg01 ~ displacement + weight + acceleration, data = trainSet)
predictions <- predict(lda_fit, testSet)
test_error <- mean(predictions$class != testSet$mpg01)
test_error
```
...
Your code:
```{r,echo=TRUE}
qda_fit <- qda(mpg01 ~ displacement + weight + acceleration, data = trainSet)
predictions <- predict(qda_fit, testSet)
test_error <- mean(predictions$class != testSet$mpg01)
test_error
```
...
Your code:
```{r,echo=TRUE}
logit_fit <- glm(mpg01 ~ displacement + weight + acceleration, data = trainSet, family = binomial)
probabilities <- predict(logit_fit, testSet, type = "response")
predicted_classes <- ifelse(probabilities > 0.5, 1, 0)
test_error <- mean(predicted_classes != testSet$mpg01)
test_error
```
...
```{r,echo=TRUE}
library(e1071)
nb_fit <- naiveBayes(mpg01 ~ displacement + weight + acceleration, data = trainSet)
predictions <- predict(nb_fit, testSet)
test_error <- mean(predictions != testSet$mpg01)
test_error
```
